...
class InstrumentCSVLoader:
    ...
    Handles the specific format used in zerodha_market_feed/instruments.csv
    ...
    def load_instruments(self) -> List[Dict[str, str]]:
        """
        ...
        try:
            ...
            logger.error(f"Failed to load CSV file {self.csv_file_path}: {e}")
            raise
    
    def load_instruments_iterator(self) -> Iterator[Dict[str, str]]:
        """
        ...
        try:
            ...
            logger.error(f"Failed to iterate CSV file {self.csv_file_path}: {e}")
            raise
    ...
    def count_instruments(self) -> int:
        ...
        try:
            ...
            logger.error(f"Error counting instruments: {e}")
    ...
    def get_default_csv_path() -> str:
        ...
        Returns:
            Path to zerodha_market_feed/instruments.csv
        """
        # Get the project root directory
        current_file = Path(__file__)
        project_root = current_file.parent.parent.parent  # Go up from utils to instrument_data to project root
        
        csv_path = project_root / 'zerodha_market_feed' / 'instruments.csv'
        return str(csv_path)
...
def load_default_instruments() -> List[Dict[str, str]]:
    """
